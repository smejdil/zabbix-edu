...
for (( j=0; j<argc; j++ )); do
    echo "winsrv${argv[j]}";
    gcloud compute instances create winsrv${argv[j]} --image ${IMAGE_WIN} --image-project=windows-cloud --zone=europe-west4-c --metadata-from-file startup-script=./zabbix-edu/scripts/firewall-zabbix-agent.ps1
