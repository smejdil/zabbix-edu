...
mysql -u root -p${MYSQL_ROOT_PASS} -e "create database zabbix-proxy character set utf8 collate utf8_bin;"
mysql -u root -p${MYSQL_ROOT_PASS} -e "grant all privileges on zabbix-proxy.* to zabbix-proxy@localhost identified by '${ZABBIX_PROXY_MYSQL_PASS}';"
...
zcat /usr/share/doc/zabbix-proxy-mysql/schema.sql.gz | mysql -u zabbix-proxy -p${ZABBIX_PROXY_MYSQL_PASS} zabbix-proxy
...
sed -i "s/^DBName=zabbix_proxy/DBHost=zabbix-proxy/g" /etc/zabbix/zabbix_proxy.conf
sed -i "s/^DBUser=zabbix/DBUser=zabbix-proxy/g" /etc/zabbix/zabbix_proxy.conf
